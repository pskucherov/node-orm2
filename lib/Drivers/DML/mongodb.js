...
Driver.prototype.callMethod = function (method, table, params) {
	var collection = this.db.collection(table);

	if (collection && typeof collection[method] === 'function') {
		return collection[method].apply(collection, params);
	} else {
...
function checkDBMethods() {
	[
		...
		console.log(typeof opts.driver.db.collection('users')[a], a);
	});
};
